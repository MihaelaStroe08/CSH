...
document.querySelectorAll('.projects-parent > a, .submenu-parent > a, nav ul li > a[href="#"]').forEach(menu => {
  menu.addEventListener('click', e => {
    ...
    const submenulist = parentLi.querySelector('ul.submenu-right');
    if (submenulist) submenulist.classList.toggle('open');
  });
});

// 2. Inchidere meniu cand dai click in afara
...
if (cookieBanner && acceptBtn) {
  ...
  acceptBtn.addEventListener('click', () => { cookieBanner.style.display = 'none'; });
}
...
function getMaxVisible() { return window.innerWidth <= 600 ? 3 : 10; }
...
function renderPagination() {
  ...
  for (let i = start; i <= end; i++) {
    ...
    btn.textContent = i;
    btn.addEventListener('click', () => { currentPage = i; renderPagination(); renderAnunturi(); });
...
if (prevBtn) prevBtn.addEventListener('click', () => { if(currentPage>1) { currentPage--; renderPagination(); renderAnunturi(); }});
if (nextBtn) nextBtn.addEventListener('click', () => { if(currentPage<totalPages) { currentPage++; renderPagination(); renderAnunturi(); }});

// Functia de afisare anunturi
function renderAnunturi() {
  const list = document.querySelector('.anunturi-list');
  if (!list) return;
  list.querySelectorAll('.anunt').forEach(a => {
...
if (window.location.hash) {
  ...
  if (!isNaN(hashPage) && hashPage >= 1 && hashPage <= totalPages) currentPage = hashPage;
  if (parts[1]) {
